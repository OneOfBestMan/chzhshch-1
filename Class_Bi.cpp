...
Class_Bi::baseItemType_Container* Class_Bi::baseItems = (Class_Bi::baseItemType_Container*)NULL;
...
Class_Bi::Class_Bi(leiBiIter biStart, leiBiIter biEnd)
{
	Start = biStart;
	End = biEnd;
...
Class_Bi::Class_Bi(leiBiIter biStart)
{
	Start = End = biStart;
	d = Start->getDirection();
...
void Class_Bi::FenBi(bool release)
{
	if (release == false)
	{
		...
		{
			baseItemType::FenBi();
			baseItems = baseItemType::container;
		}
		if (baseItems && !container)
		{
...
bool Class_Bi::bckwdSearch(ContainerType::reverse_iterator from, leiBiIter cmpTo, Direction d)
//bool Class_Bi::bckwdSearch(int from, Class_LeiBi *cmpTo, Direction d)
{
	...
	if (result != container->rbegin())
	{
		...
		container->push_back(Class_Bi(s, cmpTo));
		return true;
	} else
	{
		// 将 cmpTo 弹出
